...
func TestDisabledFlow_GetPolicyAndHandlers_NoRequiredParams(t *testing.T) {
	t.Run("request flow disabled", func(t *testing.T) {
		...
		action := p.OnRequest(&policyv1alpha.RequestContext{
			Body: &policyv1alpha.Body{Content: []byte(`{"messages":[{"content":"hello world"}]}`)},
		}, nil)
	...
	t.Run("response flow disabled", func(t *testing.T) {
		...
		action := p.OnResponse(&policyv1alpha.ResponseContext{
			ResponseBody: &policyv1alpha.Body{Content: []byte(`{"choices":[{"message":{"content":"hello world"}}]}`)},
		}, nil)
...
func TestGetPolicy(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			pRaw, err := GetPolicy(policyv1alpha.PolicyMetadata{}, tc.params)
			if tc.expectErr {
				if err == nil {
					t.Fatalf("expected error, got nil")
				}
...
func TestMode(t *testing.T) {
	...
	if mode.ResponseBodyMode != policyv1alpha.BodyModeStream {
...
func TestValidatePayload_RequestPaths(t *testing.T) {
	...
	if _, ok := pass.(policyv1alpha.UpstreamRequestModifications); !ok {
	...
	imm, ok := fail.(policyv1alpha.ImmediateResponse)
	if !ok {
...
func TestValidatePayload_ResponsePaths(t *testing.T) {
	...
	if _, ok := pass.(policyv1alpha.UpstreamResponseModifications); !ok {
	...
	resp, ok := fail.(policyv1alpha.UpstreamResponseModifications)
	if !ok {
...
func TestValidatePayload_InvertMode(t *testing.T) {
	...
	if _, ok := within.(policyv1alpha.ImmediateResponse); !ok {
	...
	if _, ok := outside.(policyv1alpha.UpstreamRequestModifications); !ok {
...
func TestOnRequestAndOnResponse(t *testing.T) {
	...
	reqNoOp := p.OnRequest(&policyv1alpha.RequestContext{}, nil)
	if _, ok := reqNoOp.(policyv1alpha.UpstreamRequestModifications); !ok {
		t.Fatalf("expected request no-op modifications, got %T", reqNoOp)
	}
	respNoOp := p.OnResponse(&policyv1alpha.ResponseContext{}, nil)
	if _, ok := respNoOp.(policyv1alpha.UpstreamResponseModifications); !ok {
	...
	reqFail := p.OnRequest(&policyv1alpha.RequestContext{Body: nil}, nil)
	if _, ok := reqFail.(policyv1alpha.ImmediateResponse); !ok {
	...
	respFail := p.OnResponse(&policyv1alpha.ResponseContext{ResponseBody: nil}, nil)
	respMod, ok := respFail.(policyv1alpha.UpstreamResponseModifications)
	if !ok {
	...
	reqDisabled := p.OnRequest(&policyv1alpha.RequestContext{Body: &policyv1alpha.Body{Content: []byte(`{"messages":[{"content":"hello world"}]}`)}}, nil)
	...
	respDisabled := p.OnResponse(&policyv1alpha.ResponseContext{ResponseBody: &policyv1alpha.Body{Content: []byte(`{"choices":[{"message":{"content":"hello world"}}]}`)}}, nil)
