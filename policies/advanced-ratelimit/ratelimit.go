...
func (p *RateLimitPolicy) processResponseBodyV1(
	...
	// Only set headers here if at least one quota updated its result via
	// response-phase cost extraction. OnResponseHeaders already covered the
	// request-phase results for all other quotas.
	hasResponsePhaseUpdate := false
	for _, qr := range allQuotaResults {
		if _, wasStored := storedResultsMap[qr.QuotaName]; !wasStored {
			hasResponsePhaseUpdate = true
			break
		}
	}
