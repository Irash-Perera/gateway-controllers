...
func TestOnRequestAndOnResponse(t *testing.T) {
	...
	reqDisabled := p.OnRequest(&policyv1alpha.RequestContext{Body: &policyv1alpha.Body{Content: []byte(`{"text":"https://example.com"}`)}}, nil)
	...
	respDisabled := p.OnResponse(&policyv1alpha.ResponseContext{ResponseBody: &policyv1alpha.Body{Content: []byte(`{"text":"https://example.com"}`)}}, nil)
